...
+ Работа с формами - [react-hook-form](https://react-hook-form.com/), [yup](https://yup-docs.vercel.app/docs/intro)
...
```bash
/src
|--pages/             # Страницы: например, страница регистрации/домашняя страница(главная)/страницы товара/админ панель
|--modules/           # Модули со своей бизнес логикой, со своей зоной ответственности. Автономные блоки с полным циклом(UI + логика + API + состояния)
...
|--ui/                # Универстальные самые простые переиспользуемые компоненты: кнопки, input, простые компоненты. Эти Компоненты могут переиспользоваться в любой компоненте, по любому пути/домену: /admin /login /home
```

## 2 вариант. Более подробная структура на основе модульной(добавил в учебных целях более подробный формат)
...
Приложение разделено на модули, которые хранятся в папках modules. Каждая папка modules - это отдельная фича, внутри фичи следующая структура:
